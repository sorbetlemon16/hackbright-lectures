...
const Letters = (props) => {
  ...
  for (const letter of ALPHABET) {
    ...
    const shouldDisable =
      props.guessedLetters.includes(letter) || props.disableAll;

    letterBtns.push(
      <button key={letter} disabled={shouldDisable} onClick={handleClick}>
        {letter}
      </button>
    );
  }
...
class Sharkwords extends React.Component {
  ...
  render() {
    ...
    if (this.state.isGameOver && !this.gameWin()) {
      playAgain = (
        <a id="play-again">The shark got you! Click here to play again.</a>
      );
    }
